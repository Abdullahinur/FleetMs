...
def ownerSignup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Error having the form as valid')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if Super_list.objects.filter(id_number=form.cleaned_data.get('id_number')).exists():
                ...
                user_login(request, user)
                messages.success(
                    request, 'Success! Welcome to you new dahsboard {user.first_name}')
                return redirect('fleet:index')
            else:
                messages.error(
    ...
    if request.GET.get('username') and request.GET.get("password"):
        ...
        if user is not None:
            ...
            if user.roles == 'owner':
                ...
                messages.success(
                    request, 'Welcome back {request.user.first_name} to your supervisor dashboard!')
                return redirect('fleet:index')
            else:
                messages.success(
